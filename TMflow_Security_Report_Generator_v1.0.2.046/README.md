...
![Size](https://img.shields.io/badge/executable-51.3MB-blue.svg)

基於 Finite State 平台 API 的 TMflow 產品安全報告生成工具，提供現代化模組化圖形介面，支援批量生成專業的安全分析報告。
...
./TMflow_Security_Report_Generator_v1.0.2.044/TMflow_Security_Report_Generator_v1.0.2.044.exe
```
...
- **六模組架構**: 
  ...
  - 報告選項模組：設定報告類型和輸出路徑
  - 進度顯示模組：即時顯示生成進度
  - API 連接模組：管理 API 連接和憑證
  - 日誌顯示模組：記錄所有操作和狀態
- **自動連線**: 啟動時自動測試 API 連接
- **智能狀態管理**: 
  - 連接狀態即時顯示（綠色=已連接，紅色=未連接）
  - 按鈕文字動態變化（Reconnect/Disconnect）
  - 完整的斷線和重連功能
- **批量報告生成**: 
  - 同時選擇多個版本進行報告生成
